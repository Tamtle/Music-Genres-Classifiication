...
if __name__ == '__main__':
	...
	for fold in range(0,2,1):
		...
		reduce_lr = ReduceLROnPlateau(monitor = 'val_acc', factor = 0.2, patience = 4, min_lr = 1e-8, verbose=1) # reduce learning rate khi val_acc khong tang duoc nua
		...
		# warm up
		'''Constant warmup: In constant warm up , you train the model with a small learning rate for few epochs 
		(5 epochs in the paper) and then increase the learning rate to “k times learning rate” .However this approach 
		causes a spike in the training error when the learning rate is changed. The observation is visible in the (b) 
		image above. The models training error is also higher compared to the simple Linear scale rule.

		Gradual warmup: As the name suggests , you start with a small learning rate and then gradually increase it by 
		a constant for each epoch till it reaches “k times learning rate”. This approach helps the model to perform 
		better with huge batch sizes (8k in this example) , which is in par with the training error of the model trained 
		with smaller batches.'''
		# cac layer thuoc pretrained model khong warm up
		for layer in model.layers[0:-3]:
			layer.trainable = False
		model.compile(loss='categorical_crossentropy', optimizer=Adam(lr=8e-5), metrics=['accuracy'])
		model.summary()
		# chay 1 epoch de warm up
